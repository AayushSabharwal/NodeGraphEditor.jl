...
import { PAN_BUTTON, ZOOM_SPEED } from "../lib/constants";
    ...
    return (
        <>
            <svg
                ...
                width={dimensions.x}
                height={dimensions.y}
                viewBox={`${viewport.x} ${viewport.y} ${dimensions.x / zoom} ${
                    dimensions.y / zoom
                }`}
                onWheel={onZoomWheel}
            ...
            >
                <Button background="transparent" onClick={() => dispatch(zoomIn())}>
                    <AddIcon />
                </Button>
                <Text width="100%" height="100%" textAlign="center" alignItem="center">
                    {Math.round(zoom * 100) + "%"}
                </Text>
                <Button background="transparent" onClick={() => dispatch(zoomOut())}>
                    <MinusIcon />
                </Button>
            </SimpleGrid>
        </>
