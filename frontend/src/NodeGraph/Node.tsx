...
export function calculateConnectorX(parent_size: Vec2, type: ConnectorType) {
    if (type === ConnectorType.input) return CONN_RADIUS + CONN_SIDE_MARGIN;
    else return parent_size.x - (CONN_RADIUS + CONN_SIDE_MARGIN);
}

export function calculateConnectorY(index: number) {
    return (
        (index - 1) * (2 * CONN_RADIUS + CONN_Y_SPACING) + CONN_RADIUS + CONN_Y_SPACING
    );
}
...
export function calculateNodeSize(node: NodeData): Vec2 {
    ...
    return {
        x:
            Math.min(
                Math.max(node.node_name.length * NODE_CHAR_WIDTH, NODE_MIN_WIDTH),
                NODE_MAX_WIDTH
            ) +
        ...
        >
            ...
            >
                {wrappedContentString(props.node_name).map((s, i) => (
                    <tspan key={i} x="50%" dy={NODE_LINE_HEIGHT}>
                        {s}
                    </tspan>
                ))}
            </text>
